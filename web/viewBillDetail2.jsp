...
<head>
    ...
    <script type="text/javascript">
        function goBack() {
            ...
            window.top.location.href = "checkbill";
        }
    </script>
</head>
<body>
    <div class="container">
        ...
        <c:if test="${empty requestScope.data_detail}">
            <p class="no-data">No bill details found.</p>
        </c:if>

        <!-- Hiển thị bảng nếu danh sách không rỗng -->
        <c:if test="${not empty requestScope.data_detail}">
            <table>
                <thead>
                    <tr>
                ...
                <tbody>
                    <c:forEach items="${requestScope.data_detail}" var="c">
                        <tr>
                            <td>${c.getBdId()}</td>
                            <td>${c.getBillId()}</td>
                            <td>
                                <c:forEach items="${requestScope.listBook}" var="b">
                                    <c:if test="${b.getBookId() == c.getBookId()}">
                                        ${b.getBookName()}
                                    </c:if>
                                </c:forEach>
                            </td>
            ...
            <h4 class="total-money">🚀 TOTAL MONEY: ${requestScope.total_money}VNĐ</h4> 
        </c:if>
